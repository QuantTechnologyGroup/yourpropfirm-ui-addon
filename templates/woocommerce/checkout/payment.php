...
$display_payment_gateways = apply_filters( 'ypf/display-payment-gateways', true );
?>
...
<div id="payment" class="woocommerce-checkout-payment yourpropfirm-checkout">
			...
			<?php if ( ! empty( $available_gateways ) ) : ?>
				<select name="payment_method_select" id="payment_method_select">
					<?php foreach ( $available_gateways as $gateway ) : ?>
						<option value="<?php echo esc_attr( $gateway->id ); ?>" <?php selected( $gateway->chosen, true ); ?>
							data-order-button-text="<?php echo esc_attr( $gateway->order_button_text ); ?>">
							<?php echo esc_html( wp_strip_all_tags( $gateway->get_title() ) ); ?>
						</option>
					<?php endforeach; ?>
				</select>
	...
	<?php endif;
	do_action( "ypf/after-payment-gateways" );
	?>
	<div class="form-row place-order place-order-hidden">
		<noscript>
			<?php
			...
			<button type="submit"
				...
				value="<?php esc_attr_e( 'Update totals', 'woocommerce' ); ?>"><?php esc_html_e( 'Update totals', 'woocommerce' ); ?>
			</button>
		</noscript>

		<?php do_action( 'woocommerce_review_order_before_submit' ); ?>

		<!-- <?php echo apply_filters( 'woocommerce_order_button_html', '<button type="submit" class="btn-primary place-order-btn-enhanced' . esc_attr( wc_wp_theme_get_element_class_name( 'button' ) ? ' ' . wc_wp_theme_get_element_class_name( 'button' ) : '' ) . '" name="woocommerce_checkout_place_order" id="place_order" value="' . esc_attr( $order_button_text ) . '" data-value="' . esc_attr( $order_button_text ) . '">' . esc_html( $order_button_text ) . '</button>' ); // @codingStandardsIgnoreLine ?> -->
